...
var debugf = logp.MakeDebug("sniffer")
...
func New(
	...
	s := &Sniffer{
		...
		dropSniffedPacket: dropSniffedPacket,
	}
...
func (s *Sniffer) Run() error {
	...
	defer handle.Close()
	debugf("defer handle close")
	debugf("s.config.Dumpfile")
	debugf(s.config.Dumpfile)
	...
	// 		return err
	// 	}
	// 	debugf("Appendworker Factory %d", i)
	// 	decoderWorkers = append(decoderWorkers, worker)
	// 	go worker.Do(jobChan) // Start decoder worker in background
	// }
	worker, err := s.factory(handle.LinkType())
	go worker.Do(jobChan)
